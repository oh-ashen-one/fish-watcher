...
app = FastAPI(title="Fish Watcher Dashboard")
...
DASHBOARD_PORT: int = stream_config.get("port", 5555)
DASHBOARD_PASSWORD: str = stream_config.get("password") or secrets.token_urlsafe(16)
...
def get_status() -> dict:
    status: dict = {"health_score": None, "alerts_today": 0, "clips_today": 0, "last_alert": None}
    ...
    if today_file.exists():
        try:
            ...
            status.update(
                ...
                alerts_today=data.get("alert_count", 0),
                clips_today=data.get("clip_count", 0),
            )
...
def html_page(title: str, content: str, pwd: str) -> str:
...
def dashboard(p: Optional[str] = None, password: Optional[str] = None) -> str:
    _verify_password(p, password)
    pwd = p or password or ""
    s = get_status()
    clips = get_clips()[:5]
    h = f'{s["health_score"]}/100' if s["health_score"] else "—"
    la = (
        f'{s["last_alert"]["type"]}<br><small style="color:var(--dim)">{s["last_alert"]["time"]}</small>'
        if s["last_alert"]
        else "None"
    )
    ch = (
        "".join(
            f'<div class="clip-card"><video controls preload="metadata"><source src="/clips/{c["filename"]}?p={pwd}" type="video/mp4"></video><div class="info"><div class="type">{c["alert_type"]}</div><div class="meta">{c["friendly_time"]} • {c["size_mb"]} MB</div></div></div>'
            for c in clips
        )
        or '<div class="empty">📹 No clips yet</div>'
    )
    return html_page(
        "Dashboard",
        f'<h1>Dashboard</h1><div class="grid"><div class="card"><h3>🏥 Health</h3><div class="stat">{h}</div></div><div class="card"><h3>🚨 Alerts Today</h3><div class="stat">{s["alerts_today"]}</div></div><div class="card"><h3>📹 Clips Today</h3><div class="stat">{s["clips_today"]}</div></div><div class="card"><h3>⏰ Last Alert</h3><div style="font-size:1.2rem">{la}</div></div></div><h2>Recent Clips</h2><div class="clip-grid">{ch}</div>',
        pwd,
    )


@app.get("/clips", response_class=HTMLResponse)
def clips_page(p: Optional[str] = None, password: Optional[str] = None) -> str:
    _verify_password(p, password)
    pwd = p or password or ""
    clips = get_clips()
    ch = (
    ...
    return html_page(
        "Clips",
        f'<h1>Clips</h1><p style="color:var(--dim);margin-bottom:30px">{len(clips)} clips</p><div class="clip-grid">{ch}</div>',
        pwd,
    )


@app.get("/live", response_class=HTMLResponse)
def live_page(p: Optional[str] = None, password: Optional[str] = None) -> str:
    _verify_password(p, password)
    pwd = p or password or ""
    return html_page(
        "Live",
        f'<h1>Live Stream</h1><div class="live-badge">● LIVE</div><div class="stream-container"><img src="/stream?p={pwd}" alt="Live"></div><p style="text-align:center;color:var(--dim);margin-top:20px">Refresh if disconnected</p>',
        pwd,
    )
...
def stream(p: Optional[str] = None, password: Optional[str] = None):
    ...
    q = stream_config.get("quality", 85)
    ...
    def gen():
        ...
        try:
            while True:
                ...
                cv2.putText(
                    f,
                    f"LIVE  {datetime.now().strftime('%b %d, %I:%M:%S %p')}",
                    (15, 30),
...
@app.get("/clips/{fn}")
def serve_clip(fn: str, p: Optional[str] = None, password: Optional[str] = None):
    _verify_password(p, password)
...
@app.get("/api/status")
def api_status(p: Optional[str] = None, password: Optional[str] = None):
    _verify_password(p, password)
    return JSONResponse(get_status())


@app.get("/api/clips")
def api_clips(p: Optional[str] = None, password: Optional[str] = None):
    _verify_password(p, password)
    return JSONResponse(get_clips())
